...
class TestRunFumbleTeamCard:
    ...
    def test_legacy_fumble_uses_team_rating(self):
        """Legacy column FUMBLE result also uses team card resolution."""
        resolver = PlayResolver()
        deck = FACDeck()
        # Create a rusher WITHOUT 12-row rushing (so it uses legacy columns)
        rusher = PlayerCard.from_dict({
            "name": "Legacy Runner",
            "number": 28,
            "position": "RB",
            "overall_grade": "B",
        ...
        # Set up a legacy inside_run column that produces FUMBLE
        rusher.inside_run = {
            str(i): {"result": "FUMBLE", "yards": 0, "td": False}
            for i in range(1, 13)
        }

        fac_card = _make_fac_card(run_number="6")
        ...
        assert result.result in ("FUMBLE", "FUMBLE_TD")
        log_text = " ".join(result.debug_log)
        assert "[FUMBLE]" in log_text
        assert "FAC drawn for recovery" in log_text
        assert "fumbles_lost_max=15" in log_text
        assert "def_fumble_adj=2" in log_text
        assert "home=True" in log_text


# ── Kickoff Return Debug Logging ─────────────────────────────────────────

class TestKickoffReturnLogging:
    """resolve_kickoff emits [KR] debug_log with returner info."""

    def test_touchback_has_kr_log(self):
        resolver = PlayResolver()
        with patch.object(Charts, 'is_kickoff_touchback', return_value=True):
            result = resolver.resolve_kickoff(returner=_make_returner())
        assert result.result == "TOUCHBACK"
        assert any("[KR]" in line for line in result.debug_log)

    def test_return_has_kr_log_with_details(self):
        resolver = PlayResolver()
        returner = _make_returner(name="Devin Hester", grade="A")
        with patch.object(Charts, 'is_kickoff_touchback', return_value=False):
            with patch.object(Charts, 'roll_kick_return', return_value=22):
                result = resolver.resolve_kickoff(returner=returner)
    ...
    def test_return_without_returner_still_logs(self):
        resolver = PlayResolver()
        with patch.object(Charts, 'is_kickoff_touchback', return_value=False):
            with patch.object(Charts, 'roll_kick_return', return_value=20):
                result = resolver.resolve_kickoff(returner=None)
        assert result.result == "RETURN"
        log_text = " ".join(result.debug_log)
        assert "[KR]" in log_text
...
# ── Punt Return Debug Logging ────────────────────────────────────────────

class TestPuntReturnLogging:
    """resolve_punt emits [PR] debug_log with returner info."""

    def test_punt_return_has_pr_log(self):
        resolver = PlayResolver()
        punter = _make_punter()
        returner = _make_returner(name="Tyreek Hill")
        # Use legacy path (no punt_column)
        with patch.object(Charts, 'check_fair_catch', return_value=False):
            with patch.object(Charts, 'roll_punt_return', return_value=5):
                with patch('random.random', return_value=0.9):  # not inside 20
                    result = resolver.resolve_punt(punter, returner=returner)
        log_text = " ".join(result.debug_log)
        assert "[PR]" in log_text
        assert "Tyreek Hill" in log_text
        assert "Base return yards=" in log_text

    def test_fair_catch_logged(self):
        resolver = PlayResolver()
        punter = _make_punter()
        returner = _make_returner(name="Fair Catcher")
        with patch.object(Charts, 'check_fair_catch', return_value=True):
            with patch('random.random', return_value=0.9):  # not inside 20
                result = resolver.resolve_punt(punter, returner=returner)
        log_text = " ".join(result.debug_log)
        assert "[PR]" in log_text
        assert "Fair catch" in log_text

    def test_inside_20_logged(self):
        resolver = PlayResolver()
        punter = _make_punter()
        returner = _make_returner()
        with patch('random.random', return_value=0.1):  # triggers inside 20 (< 0.3)
            result = resolver.resolve_punt(punter, returner=returner)
        log_text = " ".join(result.debug_log)
        assert "[PR]" in log_text
